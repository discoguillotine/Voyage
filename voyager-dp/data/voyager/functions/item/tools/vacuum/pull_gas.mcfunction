...
execute as @s[tag=voyager.entity.gas.pestilent] if entity @a[distance=..2,tag=voyager.interact,predicate=!voyager:item/tools/vacuum/capacity_16] run function voyager:item/tools/vacuum/catch/pestilent_gas